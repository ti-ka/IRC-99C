...
class Scantron{
    ...
    public function display(){
        $str = '<div class="question-set col-lg-4 col-md-6 col-sm-12 col-xs-12">';
        //$str .= '<div class="col-xs-12 question">'.$this->question.'</div>';
        $str .= '<div class="col-xs-12 answers">';
        $str .= '<div class="col-xs-2 question-nr">'.$this->questionNr.'</div>';
        //$str .= '<div class="col-xs-8">';

        $width = 100/$this->answerCount;

        for($i = 1; $i <= $this->answerCount; $i++){
            $str .= '<div class="answer col-xs-2"
                data-type="'.$this->answerType.'"
                data-question="'.$this->questionNr.'"
...
class Quiz{
    ...
    public function save($name,$wNr,$answers){
        $time       = new DateTime('now');
        try{
            $data = @file_get_contents($this->saveTo);
            if($data == ""){
                $data .= "Date/Time\tW Number\tName\tCorrect\tAnswers\tBinary Answers";
            }
